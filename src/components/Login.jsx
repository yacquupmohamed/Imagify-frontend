        ...
        try {
            ...
            if(state === 'Login'){
                ...
                if(data.success){
                    setToken(data.token)
                    setUser(data.user)
                    localStorage.setItem('token', data.token) // shaki weyn 
                    setShowLogin(false)
                }else{
  ...
  return (
    <div className='fixed top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center'>
        ...
        className='relative bg-white p-10 rounded-xl text-slate-500'>
            ...
            <p className='text-sm'>Welcome back! Please sign or login in to continue</p>

            {/* input one name*/}
            { state !== 'Login' && <div className='border px-6 py-1 flex items-center gap-2 rounded-full mt-5'>
                <img src={assets.profile_icon} alt="" className='h-6.5'/>
            ...
            {/* input two email*/}
            <div className='border px-6.5 py-1 flex items-center gap-2 rounded-full mt-5'>
                <img src={assets.email_icon} alt="" className='h-3.5'/>
            ...
            <div className='border px-6.5 py-1 flex items-center gap-2 rounded-full mt-5'>
                <img src={assets.lock_icon} alt="" className='h-4.5'/>
